...
This version offloads inference to Roboflow's serverless cloud infrastructure.
...
def start_pipeline():
    """Start the Roboflow inference pipeline using cloud inference."""
    # NOTE: Do NOT set LOCAL_INFERENCE_API_URL - this will use Roboflow's cloud
    ...
    print("Camera: USB webcam (device 0)")
    print("Inference: Roboflow serverless cloud")

    pipeline = InferencePipeline.init_with_workflow(
        api_key=os.environ.get("ROBOFLOW_API_KEY"),
        workspace_name="edss",
        workflow_id="count-milk-alerts",
        video_reference=0,  # USB webcam - change to "/dev/video0" if needed
        max_fps=5,  # Reduced FPS for Raspberry Pi 4 performance
        on_prediction=my_sink
    )

    print("Pipeline initialized. Starting video stream...")
    pipeline.start()
    pipeline.join()
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("Configuration:")
    print("  - Inference: Roboflow Cloud (serverless)")
    print("  - Camera: USB webcam")
    print("  - Max FPS: 5 (optimized for Pi 4)")
    print("=" * 60)
